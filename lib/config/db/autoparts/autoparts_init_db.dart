  ...
  Future<void> seeder() async {
    ...
    await db.transaction((txn) async {
      ...
      for (var brand in AutopartsSql.getSeedAutopartBrand()) {
        await txn.insert('AutopartBrand', {
          'id': brand.id,
          'name': brand.name,
          'logo': brand.logo,
        }, conflictAlgorithm: ConflictAlgorithm.ignore);
      ...
      for (var typeInfo in AutopartsSql.getSeedAutopartTypeInfo()) {
        await txn.insert('AutopartTypeInfo', {
          'id': typeInfo.id,
          'name': typeInfo.name,
      ...
      for (var asset in AutopartsSql.getSeedDataAupartAsset()) {
        await txn.insert('AutopartAsset', {
          'id': asset.id,
          'asset': asset.asset,
